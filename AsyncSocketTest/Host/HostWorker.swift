...
class HostWorker: NSObject {
    ...
    func startBroadcast(netServiceBlock: @escaping ((Bool,String?) -> Void), socketBlock: @escaping ((String, GCDAsyncSocket) -> Void) , incominngValueBlock:
        ...
        if let _ = try? hostSocket.accept(onPort: Network.NETSERVICE.host_port) {
            // Initialize Service
            service = NetService(domain: "local.", type: Network.NETSERVICE.type, name: Network.NETSERVICE.name, port: Int32(hostSocket.localPort))
            // Configure Service
...
extension HostWorker: NetServiceDelegate {
...
extension HostWorker: GCDAsyncSocketDelegate {
    ...
    func socket(_ sock: GCDAsyncSocket, didAcceptNewSocket newSocket: GCDAsyncSocket) {
      //  print("Accepted New Socket from \(String(describing: newSocket.connectedHost)):\(newSocket.connectedPort)")
        ...
        sendValue(str: Network.NETSERVICE.hostSoc)
    }
